...
var WordpressGenerator = yeoman.Base.extend({
  initializing: {
    ...
    upgradeNotice: function() {
      ...
      // get existing version from group vars (if available)
      try {
        var groupVars = yaml.safeLoad(this.fs.read(path.join(this.env.cwd, 'lib', 'ansible', 'group_vars', 'all')));

        if (groupVars.hasOwnProperty('evolve_version')) {
          upgradeVersion = groupVars.evolve_version;
        }
      } catch(e) {};

      // failing that, get existing version from bower json (if available)
      if (!upgradeVersion) {
        try {
          var bowerJson = this.fs.readJSON(path.join(this.env.cwd, 'bower.json'));

          if (bowerJson.dependencies.hasOwnProperty('evolution-wordpress')) {
            upgradeVersion = bowerContents.dependencies['evolution-wordpress'];
          }
        } catch (e) {};
      }

      // show upgrade notice for evolution 1.x sites
      if (upgradeVersion && !semver.satisfies(upgradeVersion, '2.x')) {
        this.log([
          chalk.bgRed.bold.underline('!!! Evolution 2.0 Upgrade Notice !!!'),
          chalk.green('You are regenerating an Evolution 1.0 site with ' + chalk.bold('significant and potentially breaking changes') + '...'),
          chalk.yellow.bold('After regenerating this site, ' + chalk.underline('you MUST re-provision any existing servers before deployment') + ' or they may not continue to function!'),
          '',
        ].join("\n"));
      }
    },
    ...
    checkDeps: function() {
      ...
      var deps = [
        { bin: 'ansible', args: ['--version'], test: function (out) { var ver = out.match(/ansible ((?:\d+[.]){2}\d+)/)[1]; return semver.satisfies(ver, '>=2.0'); } },
        { bin: 'bundler', args: ['--version'] },
        { bin: 'npm',     args: ['--version'] },
        { bin: 'sshpass', args: ['-V'] },
        { bin: 'yo',      args: ['--version'], test: function (out) { var ver = out.match(/((?:\d+[.]){2}\d+)/)[1]; return semver.satisfies(ver, '>=1.7.1'); } },
      ];
      ...
      var exited  = function(dependency, proc, err) {
        ...
        // run test, when provided, against stdout/err
          ...
          if (dependency.hasOwnProperty('test')) {
            ...
            // warn (rather than bail) on dep with insufficient version
            if (!dependency.test(output)) {
              this.log.skip('Warning: found ' + chalk.green(dependency.bin) + ', with insufficient version: ' + chalk.red(output));
            }
          }
        }
  ...
  prompting: {
    ...
    promptForSsl: function() {
      ...
      try {
        var provisionYml = yaml.safeLoad(this.fs.read(path.join(this.env.cwd, 'lib', 'ansible', 'provision.yml')));
        if (provisionYml[0].roles.indexOf('pound') == -1) {
          if (!provisionYml[0].roles.find(function(el) { return el.hasOwnProperty('role') && el.role == 'pound'; })) {
            defaultSsl = false;
          }
        }
      } catch(e) {};
    ...
    promptForWww: function() {
      ...
      var existing = function() {
        try {
          ...
          return groupVars.hasOwnProperty('www') && !!groupVars.www;
        } catch(e) {};
      }.bind(this);
    ...
    promptForNewrelic: function() {
      var existing = function() {
        try {
          ...
          if (groupVars.hasOwnProperty('monitoring') && groupVars.monitoring.hasOwnProperty('newrelic')) {
            return groupVars.monitoring.newrelic;
          }

          if (groupVars.hasOwnProperty('newrelic_api_key')) {
            return groupVars.newrelic_api_key;
          }
        } catch(e) {};
      }.bind(this);
    ...
    promptForDatadog: function() {
      var existing = function() {
        try {
          var groupVars = yaml.safeLoad(this.fs.read(path.join(this.env.cwd, 'lib', 'ansible', 'group_vars', 'all')));
          if (groupVars.hasOwnProperty('datadog_api_key')) {
            return groupVars.datadog_api_key;
          }
        } catch(e) {};
      }.bind(this);
    ...
    promptForRoles: function() {
      try {
        ...
        WordpressGenerator.roles.forEach(function (role, index) {
          var roleName = role.value.split(' ')[0];
          if (provisionYml[0].roles.indexOf(roleName) != -1) {
            WordpressGenerator.roles[index].checked = true;
          }
          else if (provisionYml[0].roles.find(function(el) { return el.hasOwnProperty('role') && el.role == roleName; })) {
            WordpressGenerator.roles[index].checked = true;
          }
          else {
    ...
    promptForAutoUpdate: function() {
      ...
      var existing = function() {
        try {
          var groupVars = yaml.safeLoad(this.fs.read(path.join(this.env.cwd, 'lib', 'ansible', 'group_vars', 'all')));
          if (groupVars.hasOwnProperty('wordpress__autoupdate') && groupVars.wordpress__autoupdate.match('/^(major|minor)$/')) {
            return groupVars.wordpress__autoupdate;
          }
        } catch(e) {};
      }.bind(this);
    ...
    ask: function() {
      return this.prompt(this.prompts).then(function(props) {
        this.props = props;
      }.bind(this));
    },
  },
  ...
  cleanupBower: function () {
    ...
    try {
      ...
      if (rcContents.scripts.postinstall == './bower_components/evolution-wordpress/lib/yeoman/bin/postinstall') {
        ...
        if (!Object.keys(rcContents.scripts).length) {
          delete rcContents.scripts;
        }

        if (!Object.keys(rcContents).length) {
    ...
    // remove old bower deps for evolution
    try {
    ...
    // remove old bower json, if no deps remain
    try {
      var bowerfile = path.join(this.env.cwd, 'bower.json');
      var bowerContents = this.fs.readJSON(bowerfile);
      if (!bowerContents.hasOwnProperty('dependencies') || Object.keys(bowerContents.dependencies).length < 1) {
        fs.unlinkSync(bowerfile);
      }
    } catch (e) {};
