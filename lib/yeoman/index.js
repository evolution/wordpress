...
var WordpressGenerator = yeoman.Base.extend({
  ...
  promptForPrivate: function() {
    ...
    var existing = function() {
      try {
        var bower = this.fs.readJSON(bowerFile);
  ...
  promptForName: function() {
    var existing = function() {
      try {
        var bower = this.fs.readJSON(path.join(this.env.cwd, 'bower.json'));
  ...
  symlinkEvolutionWordPress: function() {
    ...
    this.log.info('symlinking from', chalk.red(srcpath), 'to', chalk.blue(dstpath));

    fs.mkdirsSync(path.dirname(dstpath));
    fs.ensureSymlinkSync(srcpath, dstpath);
  },

  bootstrapBower: function() {
    // bower files we'll be bootstrapping
    var bowerJson = this.destinationPath('bower.json');
    var bowerRc   = this.destinationPath('.bowerrc');

    // minimum viable json for bower install + postinstall
    var bowerBootstrap = {
      "name": this.props.name,
      "dependencies": {
        "evolution-wordpress": (this.options.dev ? 'master' : '~' + this.pkg.version),
        "wordpress": this.props.wordpress,
      }
    };

    // backup existing bower files as necessary
    var backupBowerJson = fs.existsSync(bowerJson) ? fs.readFileSync(bowerJson) : null;
    var backupBowerRc   = fs.existsSync(bowerRc) ? fs.readFileSync(bowerRc) : null;

    // bootstrap bower files
    fs.writeFileSync(bowerJson, JSON.stringify(bowerBootstrap));
    fs.copySync(this.templatePath('_bowerrc'), bowerRc, {clobber:true});

    // bower install from bootstrapped files
    cp.spawnSync('bower', ['install'], {cwd: this.env.cwd, env: process.env, stdio: 'inherit'});

    // restore existing bower files as necessary
    backupBowerJson ? fs.writeFileSync(bowerJson, backupBowerJson) : fs.unlinkSync(bowerJson);
    backupBowerRc ? fs.writeFileSync(bowerRc, backupBowerRc) : fs.unlinkSync(bowerRc);
  },
  ...
  scaffold: function() {
    ...
    this.templates.forEach(function(file) {
      this.log.info('scaffolding', chalk.red(file));
      this.template(file, file.replace(/(^|\/)_/, '$1.'));
      this.log.info('scaffolded', chalk.blue(file));
    }.bind(this));
  },
  ...
  install: {
    ...
    installBower: function () {
      ...
      this.log.info(chalk.green('Reinstalling bower...'));

      this.spawnCommand('bower', ['install'])
        .on('error', function (err) {
          this.log.error('Could not run', chalk.yellow('bower install'));
        }.bind(this))
        .on('exit', done)
      ;
    },
